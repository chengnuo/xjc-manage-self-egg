...
class TopicsController extends Controller {
  ...
  async show() {
    const { ctx } = this;
    const result = await ctx.service.friendPortrait.show({
      id: ctx.params.id,
    });
  ...
  async index() {
    ...
    const list = await ctx.service.friendPortrait.list(listData); // 列表
    const total = await ctx.service.friendPortrait.total(whereData); // 条数
  ...
  async create() {
    ...
    if (result && result.title === ctx.request.body.title) {
      ctx.body = {
        status: 201,
        message: '朋友画像标题已存在，请使用其他朋友画像标题',
      };
    } else {
      const id = await ctx.service.friendPortrait.create(Object.assign({}, ctx.request.body, {
  ...
  async update() {
    ...
    const result = await ctx.service.friendPortrait.update(Object.assign({ id }, ctx.request.body));
  ...
  async destroy() {
    const { ctx } = this;
    const id = ctx.params.id;
    const result = await ctx.service.friendPortrait.destroy({
      id,
    });
    if (result.affectedRows === 1) {
      ctx.body = {
