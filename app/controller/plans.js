...
class TopicsController extends Controller {
  ...
  async show() {
    const { ctx } = this;
    const result = await ctx.service.plans.show({
      id: ctx.params.id,
    });
  ...
  async index() {
    ...
    const list = await ctx.service.plans.list(listData); // 列表
    const total = await ctx.service.plans.total(whereData); // 条数
  ...
  async create() {
    ...
    if (result && result.title === ctx.request.body.title) {
      ...
      const id = await ctx.service.plans.create(Object.assign({}, ctx.request.body, {
        uid: ctx.session.user.id, // 从sesstion 里面获取
      }));
      ctx.body = {
        status: 200,
  ...
  async update() {
    ...
    const result = await ctx.service.plans.update(Object.assign({ id }, ctx.request.body));
  ...
  async destroy() {
    const { ctx } = this;
    const id = ctx.params.id;
    const result = await ctx.service.plans.destroy({
      id,
    });
    if (result.affectedRows === 1) {
      ctx.body = {
