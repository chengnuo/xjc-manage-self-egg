...
class TopicsController extends Controller {
  ...
  async show() {
    const { ctx } = this;
    const result = await ctx.service.tool.show({
      id: ctx.params.id,
    });
  ...
  async create() {
    const { ctx } = this;
    const result = await this.app.mysql.get('tool', { title: ctx.request.body.title });
    ...
    if (result && result.title === ctx.request.body.title) {
      ...
      const id = await ctx.service.tool.create(Object.assign({}, ctx.request.body, {
  ...
  async update() {
    ...
    const result = await ctx.service.tool.update(Object.assign({ id }, ctx.request.body));
  ...
  async destroy() {
    const { ctx } = this;
    const id = ctx.params.id;
    const result = await ctx.service.tool.destroy({
      id,
    });
    if (result.affectedRows === 1) {
      ctx.body = {
